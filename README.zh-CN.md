...
**先停止运行中的进程。** 实盘运行（非 `--record-only`）启动时会把进程
pid 写入 `/tmp/entropy-arb-<symbol>-<base>-<hedge>.pid`，退出时删除。
对同一组合执行 `entropy-arb flatten` 时会读取该文件，先停止运行中的
bot（SIGTERM，宽限后仍未退出则 SIGKILL）再平仓——避免清仓与引擎自己的
下单互相竞争。pid 文件从不阻碍交易：写入失败只是警告；flatten 在发送
信号前会用 `ps` 核实记录的进程确实是 entropy-arb，pid 已消失或被其他
程序复用时直接忽略。
