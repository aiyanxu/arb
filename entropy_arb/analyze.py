...
Lives inside the package so the installed `entropy-arb` entry point can
serve it directly (the `tools/` directory ships with the repo, not the
wheel); tools/analyze.py is a thin wrapper over this module.
...
Usage:
    entropy-arb analyze                             # logs/minutes.duckdb, all combos
...
try:
    from .recorder import TABLE_PREFIX
except ImportError:   # script-style execution (python entropy_arb/analyze.py)
